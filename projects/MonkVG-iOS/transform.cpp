...
Transform::Transform(const View &view_)
    : view(view_)
{
}
...
const double Transform::getX()
{
    return state.x;
}

const double Transform::getY()
{
    return state.y;
}
...
void Transform::setBearing(double degrees, const Duration& duration) {
    setAngle(-degrees * M_PI / 180, duration);
}

void Transform::setBearing(double degrees, double cx, double cy) {
    setAngle(-degrees * M_PI / 180, cx, cy);
}

double Transform::getBearing() const {
    return -getAngle() / M_PI * 180;
}
