...
bool SVG::read(std::string &data) { return read(data.c_str()); }
...
void SVG::nextState(char **c, char *state) {
    ...
    if (isalpha(**c)) {
        *state = **c;
        ...
        if (islower(
                *state)) { // if lower case then relative coords (see SVG spec)
...
void SVG::parse_path_d(std::string &d) {
    ...
    while (state != 'e') {
        ...
        default:
            ...
            // TODO: figure out the next state if we don't handle a particular
            // state or just dummy handle a state!
            c++;
            break;
        }
    }
...
// semicolon-separated property declarations of the form "name : value" within
// the ‘style’ attribute
void SVG::parse_path_style(std::string &ps) {
    ...
    std::vector<std::string> key_values(
        std::sregex_token_iterator(ps.begin(), ps.end(), values_seperator, -1),
    ...
    //     key_value_tokens.begin(); tokenizer<char_separator<char>>::iterator v
    //     = k; v++;
    //     // cout << *k << ":" << *v << endl;
...
void SVG::parse_points(std::string &points) {
